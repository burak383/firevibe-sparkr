...
import { api, getToken, setSessionInvalidatedHandler, setToken } from '../api/client';
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // See api/client.ts's setSessionInvalidatedHandler - fires when any
  // request comes back 401 (expired/invalid token) or "banned" 403, after
  // the stored token has already been cleared. Dropping `user` to null here
  // makes RootNavigator swap back to the login stack immediately, the same
  // way a normal logout() call does.
  useEffect(() => {
    setSessionInvalidatedHandler(() => setUser(null));

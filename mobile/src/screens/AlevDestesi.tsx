...
import type { DeckUser, FireHour, SuperlikeStatus, SwipeStatus } from '../api/types';
...
export default function SparkRScreen() {
  ...
  // counter as swipeStatus above (0/day free, capped at 5/day premium -
  // see backend/src/subscription.js's consumeSuperlike).
  const [superlikeStatus, setSuperlikeStatus] = useState<SuperlikeStatus | null>(user?.superlikeStatus ?? null);
  ...
  useEffect(() => {
    if (user?.superlikeStatus) setSuperlikeStatus(user.superlikeStatus);
  }, [user?.superlikeStatus]);
  ...
  const handleSwipe = async (action: 'like' | 'pass' | 'superlike') => {
    ...
    try {
      const {
        match,
        swipeStatus: freshStatus,
        superlikeStatus: freshSuperlikeStatus,
      } = await api.swipe(current.id, action);
      setError(null); // clear any earlier failed-swipe banner now that one worked
      if (freshStatus) setSwipeStatus(freshStatus); // like - keeps the "X/10 kaldı" badge accurate without a refetch
      if (freshSuperlikeStatus) setSuperlikeStatus(freshSuperlikeStatus); // superlike - separate "X/5 kaldı" badge
      ...
      if (err instanceof ApiError && err.status === 402) {
        // Daily free-like limit reached, or the separate Super Vibe
        // allowance is used up (free: always 0, premium: 5/day) - straight
        // to the paywall instead of showing this as a generic error banner.
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.main}>
            ...
            <View style={styles.actionsRow}>
              ...
              <Pressable
                style={[styles.actionButton, styles.superButton, superlikeExhausted && styles.superButtonDisabled]}
                // If we already know today's Super Vibe allowance is used up
                // (free: always 0, premium: 5/day), skip the round trip and
                // go straight to the paywall - same place a 402 from the
                // backend would send them anyway.
                onPress={() => (superlikeExhausted ? navigation.navigate('Premium') : handleSwipe('superlike'))}
                disabled={acting}

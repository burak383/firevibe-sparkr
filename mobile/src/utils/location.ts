...
export interface DetectedLocation {
  ...
  // Raw GPS coordinates, sent to the backend alongside city/neighbourhood so
  // it can compute a REAL "X km away" between two people (see
  // backend/src/geo.js) instead of the old fixed placeholder distance.
  // Never shown in this app's own UI, and the backend never echoes it back
  // to any client - see backend/src/serialize.js.
  latitude: number;
  longitude: number;
}
